...
checkMsg( MSG, MSGLIST ) :-  checkTheMsg(MSG, MSGLIST).	
checkTheMsg( MSG, [] ):- fail.
checkTheMsg( msg( MSGID, MSGTYPE, SENDER, RECEIVER, CONTENT, SEQNUM ), [ MSGID | R ] ):-!.
checkTheMsg( MSG, [_|R] ):- checkTheMsg(MSG,R).

:- stdout -> print(" ------------ hello ------------------ ").
%%% :- checkMsg( msg( id1, b, c, d, e, 1 ), [id1,id2,id3] ).
